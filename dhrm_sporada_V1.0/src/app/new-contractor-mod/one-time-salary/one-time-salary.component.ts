...
export class OneTimeSalaryComponent implements OnInit {
  ...
  /** 
   * get payroll by plant code
   */
  getPayroll() {
    this.api.getpayrollDates(this.plant_Code).subscribe({
      next: (res: any) => {
        console.log('Payroll Res:',res);
   ...
   */
  getSalData(): void {
    this.api.getOneTimeSal(this.plant_Code).subscribe({
      next:  (res: any) => {
  ...
  calculateTotals(data: any) {
    ...
    Object.keys(totals).forEach((key) => {
      ...
      this.consolidatedData.push({
        ...
        Requested_By: requesterDetails ? requesterDetails.Emp_Name : "Unknown",
        Requested_On: requesterDetails
          ? requesterDetails.Uploaded_On
          : "Unknown",
        TotalEarning: totalEarnings,
        TotalDeduction: totalDeductions,
        Status: requesterDetails ? requesterDetails.Status : "Unknown",
  ...
  findRequester(
    requestId: number,
    data: Record<string, RequestData[]>,
  ): RequestData | null {
  ...
  handleApprove(data: any) {
    ...
    const resp = this.api.approveOneTimeSal(dataToSend).subscribe(
      (res: any) => {
        console.log(res);
  ...
  handleReject(data: any) {
    ...
    const resp = this.api.rejectOneTimeSal(dataToSend).subscribe(
      (res: any) => {
        console.log(res);
        this.openAlertDialog(`${res.message}`);
        this.clearAll();
  ...
  verifyData(parsedData: any) {
    try {
      ...
      const response = this.api
        .VerifyOTD_and_OTE(this.plant_Code, formData)
        .subscribe(
          (res: any) => {
            if (!res.success) {
              ...
              this.invalidEntries = [...earInvalid, ...dedInvalid].map(
                (item) => {
                  let reason = "";
                  ...
                  return {
                    ...
                    Description:
                      item.Earning_Description || item.Deduction_Description,
  ...
  SubmitVerifiedData() {
    ...
    this.api.oneTimeSalary(dataToSend).subscribe(
      (res: any) => {
        console.log(res);
        this.clearAll();
