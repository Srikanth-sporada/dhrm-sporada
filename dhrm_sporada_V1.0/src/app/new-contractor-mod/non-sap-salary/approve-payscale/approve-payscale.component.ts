...
export class ApprovePayscaleComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    /** based on approved and view route param UI renders */
  ...
  get_Trainee_Dtls(Apln_Slno: any, Con_ID: any, PayScale_ID: any) {
    console.log('api data:',Apln_Slno, Con_ID, PayScale_ID);
    this.api.getTraineDtlsSal(Apln_Slno).subscribe({
      next:(res) => {
        this.apln_list = res;
        console.log(res);
        /** get payscale based on contractor & payscale_ID */
        this.get_Payscale(Con_ID, PayScale_ID);
        /** set trainee data values as FB */
  ...
  get_Payscale(con_Id: any, PayScale_ID: any) {
    ...
    console.log('payscale API data:',data);
    this.api.getSinglePayscale(data).subscribe({
      next: (res) => {
        this.payscale_Data = res;
        console.log('payscale Data:',res);
        /** filter payscale based on route param effective date */
        this.payscale_Data = this.payscale_Data.filter((item: any) => item.Effective_Date == this.Effective_Date);
        console.log('filtered payscale:',this.payscale_Data[0]);
        /** set new payslip form data as payscale data */
  ...
  approve_Salary() {
   ...
   console.log('approve salary data:',data);
    this.api
      .approve_Edited_Salary(data, this.userEmpcode)
      .subscribe({
        next:(res: any) => {
        console.log('approve salary response:',res);
  ...
  openConfirmDialogWithReason(message: string, data: any): void {
    ...
    /** get reason from dialog modal */
    dialogRef.afterClosed().subscribe((result) => {
      if (result.result) {
        console.log(result);
        console.log("Dialog result:", result.result);
        console.log("Dialog reason:", result.reason);
        console.log('data from function param:',data);

        const reject = {
          data: data, // data from function param
          Reject_reason: result.reason,
          rejected_by: this.userEmpcode,
        };

        console.log('reject api data:',reject);
        /** reject API */
        this.api.rework_Emp_Salary(reject).subscribe({
          next: (res: any) => {
