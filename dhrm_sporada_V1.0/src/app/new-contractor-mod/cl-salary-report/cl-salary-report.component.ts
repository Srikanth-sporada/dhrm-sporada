...
export class CLSalaryReportComponent implements OnInit {
  ...
  all:any;
  reportType: any = [
    { name: "One Time Earnings & Deduction Salary Report", code: "OTMSAL", days: 365 ,plant:'All'},
    { name: "Previous Payroll Salary Report", code: "PRSAL", days: 365 ,plant:'All'},
    { name: "Current  Payroll Salary Report", code: "CTSAL", days: 365 ,plant:'All'},
  ]
...
getContra(){
  ...
  getData() {
    ...
    if (this.monthReport.includes(this.selectedReportType)) {
      ...
      const selectedMonthInWords = moment().month(selectedMonth - 1).format('MMMM');
    ...
    this.clApi.clSalReports(data).subscribe((resp: any) => {
      ...
      if (resp.status === 'success') {
       ...
       else if (Array.isArray(resp.data) && resp.data[0].length === 0 && resp.data[1].length === 0) {
          this.messageService.add({severity:'info',summary:'No Data Found!'})
          // alert('No data found');
          this.loading = false;
        }
...
export_excel_New(data: any[]) {
  const type = this.reportType.find((element: any) => element.code === this.selectedReportType);
  ...
  if (type.code === 'PRSAL' || type.code === 'CTSAL') {
    ...
    const contractorName = this.Con_list.find((element: any) => element.Con_Id === this.selectedContractor)?.Cont_company_name || 'All';
    ...
    XLSX.utils.sheet_add_json(worksheet2, mappedData, { header: Object.values(headerSheet1), origin: 'A6' });
    ...
    headers.forEach((header, index) => {
      const cellAddress = XLSX.utils.encode_cell({ c: index, r: lastRowNumber });
      if (worksheet2[cellAddress]) {
    ...
    for (let r = 6; r <= lastRowNumber; r++) {
      commaColumns.forEach((col) => {
        ...
        if (worksheet2[cellAddress]) {
          worksheet2[cellAddress].s = { ...(worksheet2[cellAddress].s || {}), ...commaStyle };
        }
      });
    }
...
const existingColumns = Object.keys(headerMapping).filter(key => sheet2[0].hasOwnProperty(key));
  ...
  const worksheet = XLSX.utils.json_to_sheet(formattedData, { header: existingColumns.map(key => headerMapping[key]) });
  ...
  const numericColumns = [
    'Stipend_Pay', 'Basic_Pay', 'DA_Pay', 'HRA_Pay', 'Double_OT_Pay', 'Triple_OT_Pay',
    'Leave_Salary_Pay', 'Washing_Allowance_Pay', 'Monthly_Bonus_Pay', 'Sat_and_Mon_Incentive_Pay',
    'Monthly_Attn_Incentive_Pay', 'Special_Allowance_Pay', 'Night_Shift_Pay', 'Skilled_Allowance_Pay',
    'Amenities_Allowance_Pay', 'Other_Allowance_1_Pay', 'Other_Allowance_2_Pay', 'Other_Allowance_3_Pay',
    'Other_Allowance_4_Pay', 'Arrears_Pay', 'Bonus_Pay', 'Other_Earnings_Pay', 'Retention_Incentive_Pay',
    'Total_Earnings_Pay', 'Canteen_Pay', 'Professional_Tax_Pay', 'Transport_Pay', 'WC_Policy_deduction_Pay',
    'Insurance_Pay', 'Other_Deduction_1_Pay', 'Other_Deduction_2_Pay', 'Other_Deduction_3_Pay',
    'Other_Deduction_4_Pay', 'LWF_Pay', 'Shoe/Goggles_Pay', 'Other_Deductions_Pay', 'Uniform/Coat/T-Shirt_Pay',
    'Employee_PF_Pay', 'Employee_ESI_Pay', 'Total_Deductions_Pay', 'Net_Take_Home_Pay', 'NSDC_Contribution_Pay',
    'Labour_Welfare_Fund_Pay', 'Uniform_Charges_Pay', 'Workmen_Compensation_Pay', 'Insurance_Premium_Pay',
    'Learning_Fees_Pay', 'Emp_Comp_Ins_Pay', 'Higher_Education_Fee_Pay', 'Employer_PF_Pay', 'Employer_ESI_Pay',
    'Servive_Charge_Pay', 'Total_Contractor_Pay', 'ToTal_Base_Value_Pay', 'Service_Tax_Val_Pay', 'Paid_To_Contractor'
  ];
    ...
    existingColumns.forEach((key, colIndex) => {
      ...
      if (worksheet[cellAddress]) {
        worksheet[cellAddress].s = {
          ...
          alignment: { horizontal: numericColumns.includes(key) ? "right" : "left", vertical: "center" },
          border: {
            top: { style: "thin", color: { rgb: "000000" } },
            bottom: { style: "thin", color: { rgb: "000000" } },
            left: { style: "thin", color: { rgb: "000000" } },
  ...
  const centerBoldColumns = ['Calender_Days', 'Working_Days', 'Sundays', 'Holidays', 'Paid_Days', 'Absent_Days','Present_Days','Eligible_Sunday','Emp_Working_day',
    'OT_Hrs_Double', 'OT_Hrs_Trible', 'Night_SHift_Count','Worked_Sat_Mon', 'Canteen_Deduction_Days', 'Transport_Deduction_Days'];
    ...
    const columnWidths = existingColumns.map(key => {
      const maxLength = Math.max(
        ...
        ...sheet2.map((row: any) => (row[key] ? row[key].toString().length : 0))
      );
...
const selectedMonthInWords = moment().month(selectedMonth).format('MMMM');
    ...
    XLSX.writeFile(workbook, `${contractorName}- ${selectedMonthInWords}-${selectedYear} Payroll Report.xlsx`);
  } 
  
  else   if (type.code === 'OTMSAL' ){
    const type1 = this.reportType.find((element: any) => element.code === this.selectedReportType)?.name||'';
    const contractorName = this.Con_list.find((element: any) => element.Con_Id === this.selectedContractor)?.Cont_company_name || 'All';
