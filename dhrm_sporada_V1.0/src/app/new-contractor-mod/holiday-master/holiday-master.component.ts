...
export class HolidayMasterComponent implements OnInit {
  ...
  holidayData: any = [];
  holidayType = [
    { label: "National Holiday", value: "N" },
    { label: "Festival Holiday", value: "F" },
  ];
  // Speed Dial items
  items: MenuItem[] = [
    {
  ...
  constructor(
    ...
    this.holidayForm = this.fb.group({
      ...
      holidayType: ["", { validators: [Validators.required], disabled: false }],
      holidayName: ["", { validators: [Validators.required], disabled: false }],
    });
  }
  ...
  deleteHoliday(event: Event, data: any) {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.deleteHolidayAPICall(formData);
      },
      reject: () => {
  ...
  update() {
    console.log('BEFORE:',this.holidayForm.value)
    // console.log("Update",this.holidayForm.value)
    // this.holidayForm.controls["holidayType"].setValue(
    //   this.holidayForm.value.holidayType[1],
    // );
    console.log('AFTER:',this.holidayForm.value)
  ...
  filterHolidayByPlant() {
    if (this.selectedPlant == "") {
      ...
      if (filteredHolidayData.length) {
        ...
        this.messageService.add({
          severity: "info",
          summary: `Holiday Not Found For Plant: ${this.selectedPlant}`,
        });
      }
    }
  }
  ...
  filterHoliday(event: any) {
    ...
    const filteredHolidayData = this.holidayData.filter(
      (holiday: any) =>
        holidayType == holiday.type || this.selectedPlant == holiday.plant_code,
    );
