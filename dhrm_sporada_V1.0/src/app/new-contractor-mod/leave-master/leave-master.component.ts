...
export class LeaveMasterComponent implements OnInit {
  ...
  validOpt: any;
  // selectedFile:any
  /** valid records modal Ref */
  @ViewChild("validRecords", { read: TemplateRef }) validRecordTemplateRef:
    | TemplateRef<unknown>
    | any;
  /** update recors template ref */
  @ViewChild("updateRecords", { read: TemplateRef }) updateRecordTemplateRef:
    | TemplateRef<unknown>
    | any;
  /** invalid records template ref */
  @ViewChild("invalidRecords", { read: TemplateRef }) invalidRecordTemplateRef:
    | TemplateRef<unknown>
    | any;
    /** single update template ref */
    @ViewChild("singleUpdateView", { read: TemplateRef }) singleUpdateView:
    | TemplateRef<unknown>
    | any;
  constructor(
  ...
  verifydata() {
    ...
    if (this.parsedData.length > 0) {
      ...
      this.updateReocrds = [];
      this.api.verify_leave_Mst(data).subscribe(
        (res: any) => {
          // console.log(res)
          if (!res) {
            this.get_mst_Leave_data();
            ...
            if (
              this.validOpt.validatedRecords.length === 0 &&
              this.validOpt.updateReocrds.length === 0 &&
              ...
              /** opening modal based on the record type  */
              if(this.updateReocrds.length){
                /** opening update record modal */
                console.log('Opening update record modal',this.updateRecordTemplateRef)
                this.openModal(this.updateRecordTemplateRef);
              } else if(this.nonValidatedRecords.length){
                /** opening invalid modal */
                console.log('Opening invalid record modal',this.invalidRecordTemplateRef)
                this.openModal(this.invalidRecordTemplateRef);
              }else{
                /** opening valid records modal */
                console.log('Opening valid record modal',this.validRecordTemplateRef)
                this.openModal(this.validRecordTemplateRef)
              }
              console.log(this.validOpt)
            }
          }
    ...
    this.api.submit_leave_mst(data).subscribe(
      (res: any) => {
        ...
        if (
          ...
          this.updateReocrds.length == 0 &&
          this.nonValidatedRecords.length == 0
        ) {
          this.clsoe();
        } else {
          this.showForm = true;
  ...
  openModal(templateRef:any){
    console.log('OPENING:',templateRef);
