...
export class DeclaredCompOffComponent implements OnInit {
      ...
      if (holiday.plant_code == this.plant_Code) {
        ...
        if (holidayDate === formattedSelectedDate) {
          if (holiday.type === "F") {
            this.messageService.add({
              severity: "warn",
              summary: "Already a festival holiday is declared.",
            });
          } else if (holiday.type === "N") {
            this.messageService.add({
              severity: "warn",
              summary: "Already a national holiday is declared.",
            });
          }
        }
      }
  ...
  getAltHoliday_data() {
    this.api.get_Alt_Holiday_data().subscribe({
      next: (res) => {
  ...
  onSubmit() {
    if (this.D_OffForm.valid) {
      ...
      formData.holiday_date = this.formatDate(
        this.D_OffForm.value.holiday_date,
      );
      formData.coff_date = this.formatDate(this.D_OffForm.value.coff_date);

      // Check if the selected dates match holidays
      const matchingHoliday = this.checkForMatchingHoliday(
        formData.holiday_date,
        formData.plant,
      );
      const matchingC_Off = this.checkForMatchingHoliday(
        formData.coff_date,
        formData.plant,
      );
      ...
      if (matchingHoliday && matchingC_Off) {
        ...
        this.openAlertDialog(
          "Already a holiday is declared for both Holiday Date and C-Off Date",
          "error",
        );
      } else {
  ...
  handleMatchingHoliday(matchingHoliday: any, dateType: any) {
    if (matchingHoliday.type === "F") {
      this.openAlertDialog(
        `Already a festival holiday is declared for ${dateType}. Please change ${dateType}`,
        "error",
      );
    } else if (matchingHoliday.type === "N") {
      this.openAlertDialog(
        `Already a national holiday is declared for ${dateType}. Please change ${dateType}`,
        "error",
      );
    }
  }
  ...
  update() {
    ...
    this.api.updt_Alt_Holiday(formData, this.userEmpcode).subscribe(
      (res) => {
        this.openAlertDialog(`${res}`, "check");
        this.getAltHoliday_data();
        this.hideForm();
  ...
  deleteAltHoliday(event: Event, data: any) {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.deleteAltHolidayAPICall(formData);
      },
      reject: () => {
  ...
  deleteAltHolidayAPICall(formData: any) {
    this.api.del_Alt_Holiday(formData, this.userEmpcode).subscribe(
      (res) => {
        this.messageService.add({ severity: "info", summary: `${res}` });
        this.getAltHoliday_data();
        this.hideForm();
  ...
  filterAltHolidayByPlant() {
    if (this.selectedPlant == "") {
      ...
      const filteredAltHoliday = this.altHolidayData.filter(
        (altHoliday: any) => altHoliday.plant == this.selectedPlant,
      );
      if (filteredAltHoliday.length) {
        ...
        this.messageService.add({
          severity: "info",
          summary: `Alternate Holiday Not Found For Plant: ${this.selectedPlant}`,
        });
      }
    }
  }
