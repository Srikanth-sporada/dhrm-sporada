...
@Component({
  selector: 'app-excess-hour-chart-larger',
  templateUrl: './excess-hour-chart-larger.component.html',
  styleUrls: ['./excess-hour-chart-larger.component.css']
})
export class ExcessHourChartLargerComponent implements OnInit {
@Input() date: any;
  @Input() plant: any;
  ...
  barChartOptions: ChartConfiguration["options"] | any = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: "OT Hours Greater Than 75 Hrs",
        padding: 20,
        font: {
          size: 30,
        },
  ...
  getData(){
    ...
    console.log(data)
    this.apiService.getexcesshourschart_75(data).subscribe({
      next: (response:any)=>{
    ...
    error:(error) => {
      console.error('GET EH 75 API ERROR:',error);
