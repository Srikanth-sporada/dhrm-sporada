...
export class OtherComponent implements OnInit {
  ...
  onChangeVanEligible(value: string): void {
    const transporterControl = this.forms.get('Transporter');
    const villageControl = this.forms.get('Village');
  
    if (value === 'Yes') {
      transporterControl?.enable();
      villageControl?.enable();
    } else {
      transporterControl?.disable();
  ...
  getdatabasic() {
    ...
    if (this.other[0]?.any_empl_rane == 'undefined' || this.other[0]?.prev_rane_empl == 'undefined' || this.other[0]?.existing_empl_name == 'undefined' || this.other[0]?.prev_rane_exp == 'undefined' || this.other[0]?.existing_empl_company == 'undefined' || this.other[0]?.extra_curricular == 'undefined') {
      ...
      if (this.other[0]?.extra_curricular != 'null')
        this.forms.controls['Van_Eligible'].setValue(this.other[0]?.Van_Eligible == true ? 'Yes':'No')
      if (this.other[0]?.extra_curricular != 'null')
        this.forms.controls['Transporter'].setValue(this.other[0]?.transporter_name)
      if (this.other[0]?.extra_curricular != 'null')
        this.forms.controls['Village'].setValue(this.other[0]?.village_name)
    }
    this.sendData()
  }
