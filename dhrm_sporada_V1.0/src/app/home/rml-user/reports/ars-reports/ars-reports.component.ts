...
export class ArsReportsComponent implements OnInit {
  ...
  reportType: any = [
    ...
    {
      index: 14,
      name: "Continuous 11 Days Working",
      code: "cont",
      days: 31,
      plant: "All",
    },
    ...
    {
      index: 17,
      name: "Operator Leave Balance Report",
      code: "Optr_LB",
      days: 31,
      plant: "All",
    },
    {
      index: 18,
      name: "Operator Permission Report",
      code: "PR",
      days: 31,
      plant: "All",
    },
    ...
    {
      index: 21,
      name: "Head Count Plan Vs Actual -Direct Manpower",
      code: "HCP",
      days: 365,
      plant: "All",
    },
    ...
    {
      index: 24,
      name: "Muster Attendance Summary",
      code: "MAS",
      days: 365,
      plant: "All",
    },
    ...
    {
      index: 26,
      name: "Van Delay Regularization Report",
      code: "VDR",
      days: 365,
      plant: "All",
    },
  ...
  ngOnInit() {
    ...
    if (!this.isadmin) {
      ...
      this.getPlantsForNonAdmins(plantCode);
    } else {
      this.getPlantsForAdmins();
    }
  }
  /**
   * get plant code for non admins [CHR,CFIN,OTHERS]
   * @param {any} plantCode
   *  */
  getPlantsForNonAdmins(plantCode: any) {
    this.api.getplantcode(plantCode).subscribe({
      ...
      error: (error) => {
        console.log("GET PLANT API ERROR IN ARS REPORT:", error);
        this.messageService.add({ severity: "error", summary: error.response });
      },
    });
  }
  /**
   * get plants for admins
   */
  getPlantsForAdmins() {
    this.api.getplant().subscribe({
      ...
      error: (error: any) => {
        console.log("GET PLANTS API ERROR ARS REPORT:", error);
  ...
  getData() {
    ...
    /** report format */
    let data = {
      from: this.monthReport.includes(this.selectedReportType)
      ...
      next: (resp: any) => {
        ...
        if (resp.status === "success") {
          ...
          if (Array.isArray(resp.data) && resp.data.length === 0) {
            ...
            console.log(
              "IS NON EMPTY ARRAY",
              resp?.data.some((innerArray: any) => innerArray.length == 0),
            );
  ...
  /**
   * convert API response data based on the report type
   * @param {any} data
   * it will convert the data based on the report type.-
   * For HRC report type sheet cols are dynamically genreated from the API
   */
  exportexcel(data: any) {
    ...
    if (
      ...
      var wb = XLSX.utils.book_new();
      let type = this.reportType.filter(
      ...
      if (!type || type.length === 0) {
        ...
        this.messageService.add({
          severity: "warn",
          summary: "Invalid report type selected",
        });
        return;
      }
      ...
      if (type[0].code === "HO_5or6_RPT") {
        if (this.from <= "2024-08-25") {
          ...
          sheets = [
            {
              sheetName: "5 & 6 Days Working Payroll Report",
              dataArray: data[0],
            },
          ];
        }
        ...
        let financeSheetHeaders = Object.keys(data[6][0]).reverse();
        console.log("FINANCE SHEET HEADERS:", financeSheetHeaders);
        /** remove the first element of the array in response object keys */
        financeSheetHeaders.shift();
        /** add the removed [Total] header in the array as last */
        financeSheetHeaders.push("Total");
        sheets = [
          ...
          { sheetName: "RCC Format", dataArray: data[5] },
          {
            ...
            dataArray: data[6].map((row: any) =>
              Object.fromEntries(
                financeSheetHeaders.map((col) => [col, row[col]]),
              ),
            ),
            header: financeSheetHeaders, // 👈 Add this line
          },
        ];
      }
      // new
      ...
      if (sheets.length === 0) {
        ...
        this.messageService.add({
          severity: "warn",
          summary: "No sheets defined for the selected report type",
        });
        return;
      }
      /** OT 75 DATA CHECK */
      ...
      sheets.forEach((sheet: any, index: any) => {
        ...
        if (sheet.dataArray && sheet.dataArray.length !== 0) {
          ...
          const columnWidths = Object.keys(sheet.dataArray[0] || {}).map(
            (key) => ({
