...
export class OperatorLeaveApprComponent implements OnInit {
  ...
  atLeastOneSelected(): boolean {
    return (
      this.optr_Data &&
      Array.isArray(this.optr_Data) &&
      this.optr_Data.some((data: any) => data.isSelected)
    );
  }
  ...
  submit() {
    const selectedItems = this.optr_Data.filter(
      (data: any) =>
        data.L2_Approval_Status === "Waiting for Approval" && data.isSelected
    );
