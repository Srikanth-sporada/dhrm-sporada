...
// material modal template ref
    @ViewChild('content', {read: TemplateRef}) addEmployeeTemplateRef: TemplateRef<unknown> | undefined;
  // Speed Dial items
    items: MenuItem[] = [
              {
  ...
  constructor(private fb: UntypedFormBuilder, private modalService: NgbModal, private formservice: FormService, private service: ApiService, public loader: LoaderserviceService,private messageService:MessageService,private confirmationService:ConfirmationService) {
 ...
 // angular lifecycle
  ...
  edit(a: any) {
    ...
    this.form.controls['payroll_plant'].setValue(this.employee[a].Payroll_Plant)
    ...
    this.form.controls['is_plant_head'].setValue(this.employee[a].is_plant_head)
      ...
      .subscribe({
        next: (response: any) => {
          ...
          if (response.message == 'already') {
            this.messageService.add({severity:'info',summary:'Username Already Exists!'})
          }
          else if(response.message == 'failure'){
            this.messageService.add({severity:'error',summary:'Cannot Add Employee!'})
          }
          else {
 ...
 // delete employee
  deleteEmployee(event:Event,a: any, slno: any) {
    ...
    this.confirmationService.confirm({
        target: event.target as EventTarget,
            ...
            accept: () => {this.deleteEmployeeAPICall(a,slno)},
            reject: () => {
  ...
  this.service.deleteemployee({ empl_slno: slno })
        .subscribe({
          next: (response: any) => {
            ...
            this.messageService.add({severity:'info',summary:'Employee Deleted!'})
          }
        })
  }
  // get department line
  getLineName(event: any) {
    console.log(event.value)
    this.service.getLineName({ dept_slno: event.value })
      .subscribe(
        {
          next: (response: any) => {
            console.log(response);
  ...
  filterEmployeeByPlant(){
   if(this.selectedPlant == 'all'){
    ...
    if(filteredEmployeeDataByPlant.length){
      ...
      this.employee = this.employeeData;
      this.messageService.add({severity:'info',summary:`Employee Not Found For Plant: ${this.selectedPlant}`})
    }
   }
  }
  // search employee
