...
export class PlantComponent implements OnInit {
  ...
  @ViewChild("content", { read: TemplateRef }) addPlantTemplateRef:
    | TemplateRef<unknown>
    | undefined;
  // Speed Dial items
  items: MenuItem[] = [
   ...
   */
  save() {
    this.form.controls["plant_sign"].setValue(
      this.form.controls["plant_code"].value + "_sign." + this.new,
    );
    console.log(this.form.value);

    this.service.addplant(this.form.value).subscribe({
      next: (response: any) => {
        console.log(response);
        if (response.message == "already") {
          this.messageService.add({
            severity: "info",
            summary: "Plant Code already exists",
          });
          ...
          this.messageService.add({
            severity: "info",
            summary: "Plant Added Successfully!",
          });
        } else if (response.message == "failure") {
          this.messageService.add({
            severity: "error",
            summary: "Can't Add Plant!",
          });
        }
      },
      error: (err) =>
  ...
  editSave() {
    ...
    console.log(this.form.value);
    this.service.updateplant(this.form.value).subscribe({
      next: (response: any) => {
        console.log(response);
        if (response.message == "already") {
          this.messageService.add({
  ...
  deletePlantAPICall(a: any) {
    console.log(this.dummy[a]);
    this.service.deleteplant(this.dummy[a]).subscribe({
      next: (response: any) => {
        console.log(response);
        if (response.message == "success") {
          ...
          this.messageService.add({
            severity: "info",
            summary: "Plant Deleted Successfully",
          });
        } else {
          this.messageService.add({
            severity: "error",
            summary: "Can't Delete Plant",
          });
        }
      },
    });
  ...
  upload(event: any) {
    ...
    this.service.plantupload(formData).subscribe({
      next: (res: any) => {
  ...
  searchPlantByCodeOrName(event: any) {
    ...
    const userSearchedPlant = this.plantData.filter((plant: any) => {
      if (
        plant.plant_name.toLowerCase() == searchTerm ||
        plant.pl.toLowerCase() == searchTerm ||
        plant.plant_code.toLowerCase() == searchTerm
      ) {
        return plant;
      }
    });
