...
export class ShiftComponent implements OnInit {
  ...
  getPlantData() {
    this.service.getplant().subscribe({
      ...
      error: (err) => {
        console.error('ERROR:',err);
        this.messageService.add({ severity: "error", summary: err.messaage })
      }
    });
  }
  ...
  save() {
    ...
    this.form.controls["act_tm_from"].setValue(
      moment(this.form.controls["act_tm_from"].value).format("HH:mm:ss"),
    );
    this.form.controls["act_tm_to"].setValue(
      moment(this.form.controls["act_tm_to"].value).format("HH:mm:ss"),
    );
    this.form.controls["in_tm_min"].setValue(
      moment(this.form.controls["in_tm_min"].value).format("HH:mm:ss"),
    );
    this.form.controls["in_tm_max"].setValue(
      moment(this.form.controls["in_tm_max"].value).format("HH:mm:ss"),
    );
    console.log(this.form.value);
    this.service.addshift(this.form.value).subscribe({
      next: (response: any) => {
  ...
  edit(a: any, slno: any) {
    ...
    this.form.controls["coff_eligible_hours"].setValue(
      this.shift[a].coff_eligible_hours,
    );
    console.log(this.form.value);
  ...
  editSave() {
    ...
    console.log(this.form.value);
    this.service.updateshift(this.form.value).subscribe({
      next: (response: any) => {
        console.log(response);
        if (response.message == "updated") {
          this.form.controls["act_tm_from"].setValue(
            "T" + this.form.controls["act_tm_from"].value + ".",
          );
          this.form.controls["act_tm_to"].setValue(
            "T" + this.form.controls["act_tm_to"].value + ".",
          );
          this.form.controls["in_tm_max"].setValue(
            "T" + this.form.controls["in_tm_max"].value + ".",
          );
          this.form.controls["in_tm_min"].setValue(
            "T" + this.form.controls["in_tm_min"].value + ".",
          );
          this.service.getshift().subscribe({
            next: (response: any) => {
              this.shift = response;
  ...
  deleteShift(event: Event, a: any, slno: any) {
    ...
    this.confirmationService.confirm({
      ...
      accept: () => {
        this.deleteShiftAPICall(a, slno);
      },
      reject: () => {
  ...
  deleteShiftAPICall(a: any, slno: any) {
    this.service.deleteshift({ slno: slno }).subscribe({
      next: (response: any) => {
        console.log(response);
        if (response.message == "success") this.shift.splice(a, 1);
        this.messageService.add({
          severity: "info",
  ...
  filterShiftByPlant() {
    if (this.selectedPlant == "") {
      ...
      if (filteredShiftDataByPlant.length) {
        ...
        this.messageService.add({
          severity: "info",
          summary: `Shift Not Found For Plant: ${this.selectedPlant}`,
        });
      }
    }
  }
