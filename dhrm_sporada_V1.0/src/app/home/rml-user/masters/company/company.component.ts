...
export class CompanyComponent implements OnInit, AfterViewInit {
  ...
  getCompanyList() {
    this.service.companyshow().subscribe({
      next: (response) => {
        this.dummy = response;
    ...
    this.service.companyadd(this.form.value).subscribe({
      next: (response: any) => {
        ...
        if (response.message == "already") {
          this.messageService.add({
            severity: "warn",
            summary: "Company with same code already exists",
          });
          /** refresh */
          this.getCompanyList();
          this.form.reset();
  ...
  editSave() {
    ...
    this.form.controls["modified_by"].setValue(
      sessionStorage.getItem("emp_name"),
    );
  ...
  updateCompanyDetails(updateData: any): void {
    this.service.companyedit(updateData).subscribe({
      next: (response: any) => {
        console.log(response);
        if (response.message == "success") {
          this.messageService.add({
        ...
        if (response.message == "already") {
          ...
          this.form.controls["created_on"].setValue(
            this.dummy[this.form.controls["sno"].value].created_on,
          );
          this.form.controls["created_by"].setValue(
            this.dummy[this.form.controls["sno"].value].created_by,
          );
  ...
  deleteCompanyAPICall(a: any) {
    this.service.companydel(this.dummy[a]).subscribe({
      next: (response: any) => {
        console.log(response);
        if (response.message == "success")
          this.messageService.add({
