...
export class QuestionBankComponent implements OnInit {
  ...
  constructor(private fb: UntypedFormBuilder, private service: ApiService, private active: ActivatedRoute, private router:Router,public loader:LoaderserviceService,private messageService:MessageService) {
  ...
  ngOnInit(): void {
    ...
    this.service.getModulesQa(this.username, this.plant)
    .subscribe({
  ...
  getquestions(event:any)
  {
    ...
    this.service.getQuestions_tnr({module: event.value.module_name, plant_code: sessionStorage.getItem('plantcode')})
    .subscribe(
      {
        next: (response:any)=>{
  ...
  addrow(i:any)
  {
    ...
    {
      this.messageService.add({severity:'warn', summary:'Please Select A Module!'})
  ...
  file(event:any, i:any)
  {
    ...
    formData.append("file", event.target.files[0], this.form.controls['module'].value.module_name+'_'+(i+1)+ '_picture.'+exten )

    this.questions[i].image_filename = this.form.controls['module'].value.module_name+'_'+(i+1)+ '_picture.'+exten
