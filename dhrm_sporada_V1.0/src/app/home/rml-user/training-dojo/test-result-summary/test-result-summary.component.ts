...
export class TestResultSummaryComponent implements OnInit {
  ...
  all: any;
  userDetails: any;
  testStatus: any = "in_training"; // deafult test status
  isreportingAuth: any;
  empslno: any = null;
  /** test status obj */
  statusList = [
    { label: "IN-TRAINING", value: "in_training" },
    { label: "TRAINING COMPLETED", value: "completed" },
  ];
  ...
  constructor(
    ...
    this.date.start = new DatePipe("en-US").transform(
      new Date(
  ...
  ngOnInit(): void {
    ...
    /** get trainee test summary */
    this.getTestResultSummary();
  }

  /**
   * get test result summary for reporting auth
   * @property {*} data
   */
  getTestResultSummary(){
     this.service
      ...
      .subscribe({
        next: (response) => {
          console.log('TEST RESULT SUMMARY:',response);
          this.data = response;
        },
        error: (error) => {
  ...
  exportexcel() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "test-summary");
