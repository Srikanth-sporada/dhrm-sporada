...
export class TraineeApplicationComponent implements OnInit {
  ...
  sendFormData() {
    ...
    if (this.bankForms.invalid) {
      ...
      this.service.traineeFormData(this.bankForms.value).subscribe({
        next: (response: any) => {
          ...
          if (this.errmsg.status == "newform") {
            ...
            window.alert(
              "Your application has already been registered. kindly contact HR department"
            );
            console.log(this.bankForms.value);
          }else if(this.errmsg.status == "failed"){
            alert(this.errmsg.message)
          }
        },
  ...
  getPassword(event: any) {
    if (event?.length == 10) {
      this.bankForms.controls["pass"].setValue(
        this.bankForms.controls["mobileNumber"].value.substr(-4)
      );
