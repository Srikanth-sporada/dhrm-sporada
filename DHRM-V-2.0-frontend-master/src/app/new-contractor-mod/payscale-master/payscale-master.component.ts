...
PayscaleType = 'Latest';
  ...
  constructor(private fb: FormBuilder, private api : ClamAPIService,private service : ApiService,public loader: LoaderserviceService,private dialog: MatDialog) { 
...
this.paidToContractor= this.fb.group({
  ...
  Service_chrg:[null,{validators : [Validators.required],updateOn: 'blur'}],
  Service_Percent:[null,{validators : [Validators.required],updateOn: 'blur'}],
  Service_Value:[null,{validators : [Validators.required],updateOn: 'blur'}],
  Service_Base:[null,{validators : [Validators.required],updateOn: 'blur'}] ,
...
this.Service_Details=this.fb.group({
  Service_chrg:['',{validators : [Validators.required],updateOn: 'blur'}],
  Service_Percent:['',{validators : [Validators.required],updateOn: 'blur'}],
  Service_Value:['',{validators : [Validators.required],updateOn: 'blur'}],
  Service_Tax:['',{validators : [Validators.required],updateOn: 'blur'}] ,
  status:['',{validators : [Validators.required],updateOn: 'blur'}]
    ...
    this.enableInputDPTC('Workmen_Compensation_Check','Workmen_Compensation_amt')
...
// console.log('res',res);
      ...
      //this.Con_list =  this.Con_list.filter((item:any) => item.Plant_code == plant  && item.Status=== true)
    }
    else{
      // console.log(res)
      this.Con_list =  this.Con_list.filter((item:any) => item.Plant_code == this.plant_Code  && item.Status=== true)
  ...
  this.filteredPlantArr = this.plantArr; 
      ...
      this.filteredPlantArr = this.plantArr.filter((plant:any) => plant.plant_code === this.plant_Code);
    }
  }
...
getPayScaleCode(){
 ...
 this.filterconList =  this.Con_list.filter((item:any) => item.Plant_code == plant  && item.Status=== true)
  }
...
onContractorChange(contId: number) {
  console.log(contId)
  if (contId !== null) {
    this.filterPayscaleHeader = this.PayscaleHeader.filter((option: any) => option.Con_Id === contId);
    console.log(this.PayscaleHeader.filter((option: any) => option.Con_Id === 169))
    // Assuming you want to set the payscale based on the selected contractor
    const selectedPayscale = this.filterPayscaleHeader.find(option => option.Con_Id === contId);
    console.log(selectedPayscale)
    if (selectedPayscale) {
      this.payScale.controls['payscale'].setValue(selectedPayscale.PayScale_ID.toString());
    } else {
...
//Payscale Details   
    ...
    this.newPayScaleObj.EffectiveDate  =this.formatDate(this.payScale.value.effectiveDate)
...
this.newEarningAllowance.Attendance_Bonus_amt  =this.allowance.value.Attendance_bonus 
this.newEarningAllowance.Leave_Salary_amt  =this.allowance.value.leave_Salary 
this.newEarningAllowance.Night_shift_allow_amt  =this.allowance.value.nightShift 
this.newEarningAllowance.Washing_allow_amt  =this.allowance.value.washing 
this.newEarningAllowance.Monthly_Bonus_amt  =this.allowance.value.Monthly_bonus 
this.newEarningAllowance.Sat_Mon_amt  =this.allowance.value.Sat_Mon 
this.newEarningAllowance.Night_shift_allow_amt  =this.allowance.value.nightShift 
this.newEarningAllowance.Amenities_allow_amt  =this.allowance.value.Amenities 
...
this.newDeduction.Professional_Tax_amt= this.deduction.value.Professional_Tax_amt   
this.newDeduction.Insurance= this.deduction.value.Insurance_amt   
this.newDeduction.Shoes_amt= this.deduction.value.Shoes_amt   
this.newDeduction.Uniform_Tshirt= this.deduction.value.Uniform_Tshirt_amt  
...
this.newPaidToContractor.Uniform_Charges_amt =  this.paidToContractor.value.Uniform_Chrg_amt 
this.newPaidToContractor.LWF =  this.paidToContractor.value.LWF_amt 
this.newPaidToContractor.Learning_Fee =  this.paidToContractor.value.Learning_Fee_amt 
this.newPaidToContractor.Workmen_Compensation =  this.paidToContractor.value.Workmen_Compensation_amt 
this.newPaidToContractor.Insurance_Premium =  this.paidToContractor.value.Insurance_Premimum_amt 
this.newPaidToContractor.Higher_Education_Fee =  this.paidToContractor.value.Higer_EducationFee_amt 
this.newPaidToContractor.Emp_Comp_Ins = this.paidToContractor.value.EMP_Comp_Ins_amt 


this.newPaidToContractor.Service_Type =  this.paidToContractor.value.Service_chrg
this.newPaidToContractor.Service_chrge =  this.paidToContractor.value.Service_Value 
this.newPaidToContractor.Service_percent =  this.paidToContractor.value.Service_Percent 
this.newPaidToContractor.Sc_Base =  this.paidToContractor.value.Service_Base
...
this.api.add_Payscale_Master_new(PayscaleData).subscribe((res:any)=>{
console.log(res);
...
generateGross(type: String): number {
  if (type === 'Earning') {
    const allowances = this.allowance.value.otherAllowance;
    console.log((this.payScale.value.basic || 0) ,(this.payScale.value.stipend || 0) 
    ,( this.payScale.value.DA || 0) , (this.payScale.value.HRA || 0) ,
   (this.payScale.value.conveyance || 0) , (this.allowance.value.special || 0),
   (this.allowance.value.skilled || 0) , (this.allowance.value.Attendance_bonus || 0),
   (this.allowance.value.leave_Salary || 0) , (this.allowance.value.nightShift || 0),
   (this.allowance.value.washing || 0) , (this.allowance.value.Monthly_bonus || 0 ),
   (this.allowance.value.nightShift || 0) , (allowances[0]?.amount || 0) , (allowances[1]?.amount || 0)
   ,(allowances[2]?.amount || 0) , (allowances[3]?.amount || 0));
    
    const Gross_Earning = parseFloat((this.payScale.value.basic || 0) +(this.payScale.value.stipend || 0) 
                           +( this.payScale.value.DA || 0) + (this.payScale.value.HRA || 0) +
                          (this.payScale.value.conveyance || 0) + (this.allowance.value.special || 0)+
                          (this.allowance.value.skilled || 0) + (this.allowance.value.Attendance_bonus || 0)+
                          (this.allowance.value.leave_Salary || 0) + (this.allowance.value.nightShift || 0)+
    ...
    const deduction  = this.deduction.value.otherDeduction;
console.log(this.deduction.value.canteen_amt || 0) ,(this.deduction.value.Transport_amt|| 0) ,
(this.deduction.value.WC_Policy_amt   || 0) ,(this.deduction.value.Insurance_amt   || 0) ,
(this.deduction.value.Shoes_amt   || 0) ,(this.deduction.value.Uniform_Tshirt_amt  || 0) ,
(this.deduction.value.Goggles_deduction   || 0) ,(this.deduction.value.Coat   || 0) ,
(deduction[0]?.amount   || 0) ,(deduction[1]?.amount   || 0) ,(deduction[2]?.amount   || 0) ,
(deduction[3]?.amount  || 0)

    const Gross_Deduction= parseFloat((this.deduction.value.canteen_amt || 0) +(this.deduction.value.Transport_amt|| 0) +
                        (this.deduction.value.WC_Policy_amt   || 0) +(this.deduction.value.Insurance_amt   || 0) +
                        (this.deduction.value.Shoes_amt   || 0) +(this.deduction.value.Uniform_Tshirt_amt  || 0) +
                        (this.deduction.value.Goggles_deduction   || 0) +(this.deduction.value.Coat   || 0) +
                        (deduction[0]?.amount   || 0) +(deduction[1]?.amount   || 0) +(deduction[2]?.amount   || 0) +
...
getPayscaleMaster(){
  this.api.get_Payscale_Master(this.pln).subscribe(res =>{
// console.log(res);
...
// this.filteredPayscaleData = this.payscaleData.filter((item:any) => item.Status == true)
    ...
    console.log("Payscale list not getting",error);
  })
}
getPayscaleMaster_combine(){
  this.api.get_Payscale_Master_combine(this.plant_Code,this.userEmpcode).subscribe((res:any) =>{
console.log(res);
    this.payscaleData = res
    // console.log(res);
...
filter_Latest(data:any){
  if(data ==='Latest'){
    ...
    this.filteredPayscaleData = this.payscaleData.filter((item:any) => item.Status == false)
  }
  else{
...
getPayscaleHeader(){
  this.api.get_Payscale_Header(this.plant_Code).subscribe(res =>{
    console.log(res);
...
getPayrollList(){
  this.api.getPayroll().subscribe(res =>{
    this.PayrolList = res

    this.PayrolList =  this.PayrolList.filter((item:any)=> item.Plant_code === this.plant_Code)
    // console.log(this.PayrolList[0])
  },(error)=>{
    console.log("Contractor list not getting",error);
  })
}
getContractorList(){
  this.api.getContractor_combine(this.userEmpcode).subscribe((res) =>{
    this.conList = res
//      console.log(this.conList)
//     this.conList = this.conList.filter((item:any) => (item.Status === true && item.Plant_code === this.plant_Code))
// // console.log(res)
//     this.conList = this.conList.filter((item:any) => (item.Status === true && item.Plant_code === this.plant_Code))
//     // console.log(this.conList)
  },(error)=>{
...
getPayscaleId(payScaleId: string): string | undefined {
  const foundOption = this.filterPayscaleHeader.find((option:any) => option.PayScale_ID.toString() === payScaleId);
...
console.log("Available options:", this.filterPayscaleHeader.map((option:any) => option.PayScale_ID== 1));

}

// onEdit(data:any){
//   console.log(data);
//   console.log(data.PayScale_ID);

//   this.showAdd=false;
//     this.edit = true
//   this.showPayscaleForm = true

//   this.paidToContractor.get('status')?.disable();
//   // this.payScale.get('contractor')?.disable();
//   // this.payScale.get('plant')?.disable();
//   // this.payScale.get('effectiveDate')?.disable();
//   // this.payScale.get('payscale')?.disable();

//   this.onContractorChange(data.Con_ID);

//   // Patch all values, including disabled ones
//   this.payScale.patchValue({
...
// console.log('PayScale patched values:', this.payScale.value);
...
//     Service_Value: data.Servive_Charge_Val,
...
console.log(PayscaleData);
this.api.Update_Payscale_Master(PayscaleData).subscribe((res:any)=>{
  console.log(res);
    this.openAlertDialog(`${res}`,'check')
  this. hidePayscale()
  this.resetForm()
...
// exportExcel(): void {
//   const excludedKeys = ["Effective_Date1",'Con_ID', "Effective_Date", "Tot_Deduction", "Gross_Earnings, Status ,PayScale ID"]; 
...
//         const newKey = key.replace(/_/g, ' '); // Replace underscores with spaces for better readability
...
exportExcel(): void {
  ...
  const excludedKeys = ["Effective_Date1",'EM_PF_Percent' ,'Max_PF_Amount','EMP_PF_Percent','EM_ESI_Percent', 'EMP_ESI_Percent', 'Con_ID', "Effective_Date", "Tot_Deduction", "Gross_Earnings", "Status", "PayScale ID"];
  const keyMapping = {
  ...
  if (transformedArray.length === 0) {
    console.error("No data to export. The filteredPayscaleData array is empty.");
    return;
  }
